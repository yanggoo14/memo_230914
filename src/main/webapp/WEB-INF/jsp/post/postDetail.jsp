...
<script>
	$(document).ready(function() {
		$("#saveBtn").on('click', function() {
			...
			// 이미지를 업로드 할 때에는 반드시 form태그로 구성한다.
			let formData = new FormData();
			formData.append("postId", postId); // key는 name 속성과 같다. Request Parameter명
			...
			$.ajax({
				...
				, url:"/post/update"
				, data:formData
				, enctype: "multipart/form-data"   //이미지 업로드 필수
				, processData:false  // 이미지 업로드 필수
				, contentType:false // 이미지 업로드 필수
				//response
				, success:function(data){
					if(data.code == 200){
						alert("메모가 수정되었습니다.");
						location.reload(); // 안되면 true 넣어주기
					} else {
						alert(data.error_message);
					}
				}
				, error:function(request, status, error){
					alert("글을 수정하는데 실패하였습니다.");
				}
