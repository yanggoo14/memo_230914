...
<script>
	$(document).ready(function() {
		...
		$("#loginIdCheckBtn").on('click', function() {
			...
			$.ajjax({
			   ...
			   //response
				...
				, errer(request, status, error){
					arert(data.error_message);
				}
				});			   
			}); 
		...
		$('#signUpForm').on('submit',function(e) {
			...
			// 2) AJAX: 화면 이동 되지 않음(콜백 함수에서 이동). 응답값 JSON
				...
				if(data.code == 200){
					...
					location.href = "/user/sign-in-view";  // 회원가입 후 로그인 화면으로 이동
				} else{
					//오직 실패
					alert(data.error_message);
				} 
